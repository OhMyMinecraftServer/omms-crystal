...
class EventBus(
    ...
    private fun filterMethods(
        ...
        return clazz.declaredMethods
            ...
            .filter {
                return@filter if (Modifier.isStatic(it.modifiers) == isStatic) {
                    true
                } else {
                    if (isStatic) {
                        logger.error(
                            "Expected @SubscribeEvent method {} to be static because register()" +
                                    ...
                                    " or call register() with an instance of {}, this event listener will be ignored.",
                            it,
                            clazz
                        )
                    } else {
                        logger.error(
                            "Expected @SubscribeEvent method {} to NOT be static because " +
                                    ...
                                    "Either make the method non-static, or call register({}.class), this event listener will be ignored.",
                            it,
                            clazz.simpleName
                        )
                    }
